...
Diploids are expected to be around 0.5 and tetraploids around 0.75. Since there is some population structure, most individuals show a lower value.
```{r}
...
write.csv(imput_gen, file = "geno_combined_7723.csv", quote = F) #naive file is:naive_geno.csv
```
...
```{r}
imput_gen <- read.csv("geno_combined_7723.csv")
```
