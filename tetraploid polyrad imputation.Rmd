...
```{r}
gvcf <- ("~/Documents/new_tetraploids/Mpileup/filtered_tetraploid_variants.recode.vcf")
mybgvcf <- bgzip(gvcf)
indexTabix(mybgvcf, format = "vcf")
```

```{r}
Rdat <- VCF2RADdata(mybgvcf, phaseSNPs = F, expectedAlleles = 400000, expectedLoci = 500000, min.ind.with.minor.allele = 2,min.ind.with.reads = 109 , refgenome = "/Users/pesta/Documents/potato_dm_v404_all_pm_un.fasta",taxaPloidy = 2)
```
...
theseloci <- GetLoci(Rdat)[Rdat$alleles2loc[Rdat$alleleFreq >= 0.02 & Rdat$alleleFreq < 0.75]]
theseloci <- unique(theseloci)
#peg(filename = "HindHe.jpeg")
hist(myhindheByLoc[theseloci], col = "lightgrey",
     xlab = "Hind/He", main = "Histogram of Hind/He by locus, MAF >= 0.02", breaks = 100)
...
#This will be the new dataset to work with for diversity analysis for the diploids
t_tetraploid_imput_gen <- t(tetraploid_imput_gen)

write.csv(tetraploid_imput_gen, file = "~/Documents/sandbox/tetraploid/tetra_NAIVE_geno_7723.csv")
...
imput_gen <- read.csv("~/Documents/sandbox/tetraploid/tetra_geno_7723.csv")
```


For the Naive method only
```{r}
...
#Read in imputed geotypes of choice. 
```{r}
...
```{r}
#finding those that are == 4x and 2x
...
#Assigning meaningful populations for export to GenoDive
```{r}
...
```{r}
Export_Structure(mydata_popstruct, file = "~/Documents/sandbox/tetraploid/tetra_original_structure_7723", extraCols = p)
```