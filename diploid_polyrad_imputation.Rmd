...
```{r}
gvcf <- ("/Users/pesta/Documents/new_diploids/Mpileup/filtered_diploid_variants_maxmiss6_GQ30.recode.vcf")
mybgvcf <- bgzip(gvcf)
...
hist(myhindheByLoc[theseloci], col = "lightgrey",
     xlab = "Hind/He", main = "Histogram of Hind/He by locus, MAF >= 0.02", breaks = 25)
abline(v = 0.5, col = "blue", lwd = 2)
...
write.csv(diploid_imput_gen, file = "~/Documents/sandbox/diploid/diploid_geno_7723.csv")
...
```{r}
structure_file <- read.csv("~/Documents/sandbox/diploid/diploid_geno_7723.csv", header = T, stringsAsFactors = T, row.names = NULL, fill = T)
```
...
#There were individuals that were removed due to low coverage in polyRad. use sample_name_tetras to filter out 
which ones were removed (from the imputed genotypes file)

```{r}
sample_names_diploid <- read.table("~/Documents/sandbox/diploid/diploid_geno_7723.csv", header = T, sep = ",")
...
Export to structure
```{r}
Export_Structure(mydataNaive,file = "~/Documents/sandbox/diploid/diploid_original_structure_7723", extraCols = population)
```
